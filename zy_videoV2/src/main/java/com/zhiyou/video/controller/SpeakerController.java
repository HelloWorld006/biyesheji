...
//		get方法只负责跳转到填写信息的页面
    ...
    //@PostMapping("/speaker/save")
    public String saveOrUpdate(MultipartFile speaker, SpeakerModel model, HttpServletRequest request) {

        System.out.println("saveOrUpdate得到的信息：" + model);
        String picName = speaker.getOriginalFilename();
        //获取文件名的后缀名
        String extension = FilenameUtils.getExtension(picName);
        UUID uuid = UUID.randomUUID();
        //生成新的文件名 防止重写上传时重名出现异常
        picName = uuid + "." + extension;
        String picPath = "F:\\softPackage\\utils\\apache-tomcat-8.0.38\\webapps\\ROOT\\static\\img\\teacher";
        String path = picPath + "/" + picName;
        picName="http://localhost:8080/static/img/teacher/"+picName;
        model.setSpeakerHeadUrl(picName);
        ...
        try {
            speaker.transferTo(new File(path));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return "redirect:/admin/speaker/index.do";
